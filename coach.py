...
class Coach:
    def __init__(self, reference: dict, engine, cfg):
        ...
        self.tones = {
            ...
            "throttle": "GAS  " if console else make_tone(cfg.throttle_freq, cfg.beep_ms),
        }
    ...
    def on_frame(self, f) -> None:
        ...
        # Cuanto por delante hay que avisar. Depende de la velocidad: a 250
        # km/h recorres 24 metros en el mismo tiempo que 8 metros a 80 km/h.
        # Un offset fijo en metros seria correcto en una curva y absurdo en
        # las demas.
        lead_m = f.speed_ms * self.cfg.lead + self.cfg.margin
        ...
        for i, ev in enumerate(self.events):
            ...
            if gap < 0 or gap > lead_m:
                continue

            self.fired.add(i)

            # ¿Llego a este punto a una velocidad comparable a la referencia?
            # Si no, el punto de frenada del otro piloto no es aplicable al
            # mio, y avisar seria peor que callarse.
            ...
            if mismatch > self.cfg.speed_tol:
                ...
                if self.cfg.skip_mismatch:
                    print(f"      (omitido {ev['type']} @ {ev['pos'] * 100:.2f}%){flag}")
                    continue
            else:
                flag = ""
            ...
            if self.cfg.verbose:
                print(f"{f.t:7.2f}s  {ev['type']:<8} @ {ev['pos'] * 100:6.2f}%  "
...
def main():
    ...
    ap.add_argument("--console", action="store_true",
                    help="imprimir los avisos en vez de sonarlos")
    ap.add_argument("--lead", type=float, default=0.35,
                    help="segundos de antelacion del aviso")
    ap.add_argument("--margin", type=float, default=0.0,
                    help="metros extra de antelacion (margen de seguridad)")
    ap.add_argument("--speed-tol", type=float, default=0.10,
    ...
    print(f"Referencia: {reference['track']} / {reference['car']} "
          f"({len(reference['events'])} avisos por vuelta)")
    print(f"Antelacion: {cfg.lead:.2f} s + {cfg.margin:.0f} m\n")

    coach = Coach(reference, engine, cfg)

    try:
        with make_source(cfg.replay, cfg.replay_speed) as src:
